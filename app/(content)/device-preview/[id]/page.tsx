...
export default function DeviceReadings(props: DeviceReadingsProps) {
  ...
  useEffect(() => {
    const fetchDevices = async () => {
      ...
      try {
        const response = await fetch("/api/get-device-data", {
          method: "POST",
          body: JSON.stringify({
            deviceId: (await props.params).id
          })
        });
  ...
  if (fetchError !== null || device === null) {
    return (
      <div>
        <h1>Error</h1>
        <p>
